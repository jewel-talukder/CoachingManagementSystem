...
// Helper to convert "hh:mm tt" (09:00 AM) -> "HH:mm" (09:00) for input[type=time]
...
export default function EditShiftPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchShift = async () => {
            try {
                ...
                setFormData({
                    name: data.name,
                    startTime: convertTo24Hour(data.startTime), // Convert for input
                    endTime: convertTo24Hour(data.endTime),   // Convert for input
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (formData.startTime >= formData.endTime) {
            // Warning: This simple string comparison might fail across midnight (e.g. 23:00 to 07:00).
            // Since typical school shifts usually don't cross midnight, this is acceptable for now.
            // If night shifts are needed, we should allow it. 
            // Let's relax it or just warn? For now standard check. 
