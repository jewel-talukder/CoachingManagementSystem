...
export default function EditTeacherPageClient() {
  ...
  const fetchTeacher = async () => {
    try {
      ...
      setFormData({
        ...
        qualificationId: teacher.qualificationId || 0,
        specializationId: teacher.specializationId || 0,
      ...
      addToast(error.response?.data?.message || 'Failed to load teacher', 'error');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast(error.response?.data?.message || 'Failed to update teacher', 'error');
    } finally {
      setLoading(false);
    }
  };
