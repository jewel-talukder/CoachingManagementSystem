...
export default function EditHolidayPageClient() {
  ...
  const fetchHoliday = async () => {
    try {
      setFetching(true);
      const response = await holidaysApi.getById(holidayId);
      ...
      // Parse days of week from JSON string or use the list if available
      let daysOfWeekArray: number[] = [];
      if (holiday.daysOfWeekList && Array.isArray(holiday.daysOfWeekList)) {
        daysOfWeekArray = holiday.daysOfWeekList;
      } else if (holiday.daysOfWeek) {
        try {
          daysOfWeekArray = JSON.parse(holiday.daysOfWeek);
        } catch {
          daysOfWeekArray = [];
        }
      }
      ...
      setFormData({
        branchId: holiday.branchId ? holiday.branchId.toString() : '',
        name: holiday.name || '',
        description: holiday.description || '',
        holidayType: holiday.holidayType || 'SingleDay',
      ...
      addToast(error.response?.data?.message || 'Failed to load holiday', 'error');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await holidaysApi.update(holidayId, payload);
          ...
          <select
            ...
            onChange={(e) => {
              setFormData({
                ...
                endDate: e.target.value !== 'DateRange' ? '' : formData.endDate,
                daysOfWeek: e.target.value !== 'WeeklyOff' ? [] : formData.daysOfWeek,
              });
            }}
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
          >
