...
console.log('\n📦 Post-build: Preparing standalone folder for deployment...\n');
...
if (!fs.existsSync(standalonePath)) {
  console.error('❌ Error: .next/standalone folder not found. Make sure you ran "next build" first.');
  process.exit(1);
}

console.log(`📁 Standalone folder: ${standalonePath}\n`);
...
filesToCopy.forEach(file => {
  ...
  if (fs.existsSync(sourcePath)) {
    ...
    if (fs.existsSync(destPath)) {
      console.log(`ℹ️  ${file} already exists in standalone (generated by Next.js)`);
    } else if (file === 'web.config') {
      // web.config is critical for IIS
      console.warn(`⚠️  Warning: ${file} not found! IIS deployment may fail without this file.`);
      skippedCount++;
    } else {
      console.warn(`⚠️  Warning: ${file} not found in project root, skipping...`);
      skippedCount++;
    }
  }
});
...
if (fs.existsSync(staticSourcePath)) {
  try {
    ...
    console.log('✅ Copied: .next/static folder (CSS, JS bundles, fonts, etc.)');
  } catch (error) {
    console.error('❌ Error copying static folder:', error.message);
    console.error('⚠️  WARNING: Static assets may not load correctly without this folder!');
  }
} else {
  console.warn('⚠️  Warning: .next/static folder not found in build output!');
}
...
if (fs.existsSync(envProdPath)) {
  try {
    ...
    if (envContent.includes('NEXT_PUBLIC_API_URL')) {
      ...
      fs.writeFileSync(envProdDestPath, updatedContent);
      console.log('✅ Added NEXT_PUBLIC_API_URL to .env.production');
    }
  } catch (error) {
    console.error('❌ Error processing .env.production:', error.message);
    // Create a new one with production URL
    try {
      fs.writeFileSync(envProdDestPath, `NEXT_PUBLIC_API_URL=${productionApiUrl}\n`);
      console.log('✅ Created .env.production with production API URL');
    } catch (createError) {
      console.error('❌ Error creating .env.production:', createError.message);
    }
  }
} else {
...
console.log('\n🔍 Verifying essential files in standalone folder...\n');
...
if (fs.existsSync(nextStaticPath)) {
  console.log('✅ .next/static (folder) - Static assets (CSS, JS, fonts)');
} else {
  console.error('❌ .next/static - MISSING! Static assets will not load!');
...
console.log('\n' + '='.repeat(60));
if (allFilesPresent) {
  console.log('✅ Post-build complete! Standalone folder is ready for deployment.');
} else {
  console.warn('⚠️  Post-build complete, but some files are missing. Please check above.');
}
console.log('='.repeat(60));
console.log(`\n📁 Standalone folder location:`);
